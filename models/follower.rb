...
module Follower
    def get_is_following(user_id, target_id)
        results =exec_sql("GET COUNT(*) FROM following WHERE follower_id = #{user_id} and target_id = #{target_id}")
    ...
    def self.delete_follow(user_id, target_id)
        if is_current_user?(user_id)
            exec_sql("DELETE FROM following WHERE follower_id = #{user_id} and follow_target = #{target_id}")
            return true
        end
        return false
    end
    ...
    def self.get_all_following_by_userId(user_id)
        results = exec_sql("SELECT * FROM following WHERE follower_id = #{user_id}")
        return results
    end
